...
ifndef VCPKG_ROOT
  $(error VCPKG_ROOT is not set. Please set it to your vcpkg installation directory.)
endif
TRIPLET     := x64-windows
...
clean:
	if (Test-Path '$(BUILD_DIR)') { Get-ChildItem -Path '$(BUILD_DIR)' -Exclude '.vs' | Remove-Item -Recurse -Force }
