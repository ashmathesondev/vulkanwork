...
VkPhysicalDevice Renderer::vk_physical_device() const { return physicalDevice_; }
...
uint32_t         Renderer::swapchain_image_count() const { return static_cast<uint32_t>(swapchainImages_.size()); }
...
std::optional<Renderer::FrameContext> Renderer::begin_frame() {
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) { recreate_swapchain(); return std::nullopt; }
...
void Renderer::pick_physical_device() {
    ...
    for (auto pd : devs) {
        ...
        vkGetPhysicalDeviceSurfacePresentModesKHR(pd, surface_, &pmCnt, nullptr);
...
void Renderer::create_swapchain() {
    ...
    uint32_t fmtCnt;
    vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice_, surface_, &fmtCnt, nullptr);
    std::vector<VkSurfaceFormatKHR> fmts(fmtCnt);
    vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice_, surface_, &fmtCnt, fmts.data());

    uint32_t pmCnt;
    vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDevice_, surface_, &pmCnt, nullptr);
    std::vector<VkPresentModeKHR> pms(pmCnt);
    vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDevice_, surface_, &pmCnt, pms.data());
...
void Renderer::create_image_views() {
    ...
    for (size_t i = 0; i < swapchainImages_.size(); ++i)
        swapchainImageViews_[i] = create_image_view(swapchainImages_[i], swapchainFormat_,
...
void Renderer::create_graphics_pipeline() {
    ...
    VK_CHECK(vkCreateGraphicsPipelines(device_, VK_NULL_HANDLE, 1, &ci, nullptr, &graphicsPipeline_));
...
void Renderer::create_depth_resources() {
    ...
    depthView_ = create_image_view(depthImage_, depthFmt, VK_IMAGE_ASPECT_DEPTH_BIT);
}
...
void Renderer::create_descriptor_sets() {
    ...
    for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
        VkDescriptorBufferInfo bufInfo{uniformBuffers_[i], 0, sizeof(UniformBufferObject)};
...
void Renderer::create_texture_image_view() {
    textureImageView_ = create_image_view(textureImage_, VK_FORMAT_R8G8B8A8_SRGB,
                                         VK_IMAGE_ASPECT_COLOR_BIT);
}
...
VkFormat Renderer::find_supported_format(const std::vector<VkFormat>& candidates,
