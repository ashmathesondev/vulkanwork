...
PackFile::PackFile(const std::filesystem::path& pak_path)
    : path_(pak_path)
{
    ...
    if (!f)
        throw std::runtime_error("Failed to read pack header: " + path_.string());

    if (header.magic != MAGIC)
        throw std::runtime_error("Invalid pack magic in: " + path_.string());
    if (header.version != VERSION)
        throw std::runtime_error("Unsupported pack version in: " + path_.string());
    ...
    for (uint32_t i = 0; i < header.entry_count; ++i) {
        ...
        if (!f)
            throw std::runtime_error("Failed to read TOC entry " + std::to_string(i));
...
std::vector<char> PackFile::read(std::string_view name) const {
    ...
    if (it == toc_.end())
        throw std::runtime_error("Asset not found in pack: " + std::string(name));
    ...
    f.read(compressed.data(), static_cast<std::streamsize>(entry.compressed_size));
    if (!f)
        throw std::runtime_error("Failed to read asset data: " + std::string(name));
    ...
    if (result < 0)
        throw std::runtime_error("LZ4 decompression failed for: " + std::string(name));
