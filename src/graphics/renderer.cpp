							   ...
							   const LightEnvironment& lights)
{
	...
	lastView_ = ubo.view;
	lastProj_ = glm::perspective(glm::radians(camera.fov), aspect, 0.1f, 100.0f);
...
void Renderer::draw_scene(VkCommandBuffer cmd)
{
	...
	vkCmdSetCullMode(cmd, debugDisableCulling_ ? VK_CULL_MODE_NONE
											   : VK_CULL_MODE_BACK_BIT);
	vkCmdSetFrontFace(cmd, debugFrontFace_ == 1 ? VK_FRONT_FACE_CLOCKWISE
...
void Renderer::import_gltf(const std::string& path)
{
	...
	meshes_.insert(meshes_.end(),
				   std::make_move_iterator(scene.meshes.begin()),
...
void Renderer::rebuild_material_descriptors()
{
	...
	create_material_descriptor_pool(
		static_cast<uint32_t>(materials_.size()));
...
void Renderer::create_debug_line_pipeline()
{
	...
	VK_CHECK(vkCreateGraphicsPipelines(device_, VK_NULL_HANDLE, 1, &ci,
									   nullptr, &debugLinePipeline_));
...
void Renderer::update_debug_lines(const LightEnvironment& lights)
{
	...
	constexpr VkDeviceSize bufSize = 64 * 1024;
	uint32_t maxVerts =
		static_cast<uint32_t>(bufSize / sizeof(LineVertex));
