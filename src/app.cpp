...
void App::init_window() {
    ...
    glfwSetFramebufferSizeCallback(window, [](GLFWwindow* w, int, int) {
        static_cast<App*>(glfwGetWindowUserPointer(w))->renderer.notify_resize();
    });
}
...
void App::process_input() {
    ...
    if (mouseCaptured) {
        ...
        glm::vec3 d;
        d.x = std::cos(glm::radians(camera.yaw)) * std::cos(glm::radians(camera.pitch));
        d.y = std::sin(glm::radians(camera.pitch));
        d.z = std::sin(glm::radians(camera.yaw)) * std::cos(glm::radians(camera.pitch));
    ...
    if (!io.WantCaptureKeyboard) {
        ...
        if (glfwGetKey(window, GLFW_KEY_LEFT_CONTROL) == GLFW_PRESS)  camera.position -= camera.up * v;
    }
}
...
void App::init_imgui() {
    ...
    VK_CHECK(vkCreateDescriptorPool(renderer.vk_device(), &poolCI, nullptr, &imguiPool));
