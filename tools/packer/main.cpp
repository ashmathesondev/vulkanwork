...
static void print_usage(const char* argv0) {
    std::fprintf(stderr,
        ...
        "  -b <path>   base directory for relative name computation (default: cwd)\n",
        argv0, argv0, argv0);
}
...
static int list_pak(const std::string& pak_path) {
    ...
    for (uint32_t i = 0; i < header.entry_count; ++i) {
        ...
        double ratio = entry.original_size > 0
            ? 100.0 * static_cast<double>(entry.compressed_size) / static_cast<double>(entry.original_size)
            : 0.0;
    ...
    double total_ratio = total_original > 0
        ? 100.0 * static_cast<double>(total_compressed) / static_cast<double>(total_original)
        : 0.0;
...
static int validate_pak(const std::string& pak_path) {
    ...
    for (uint32_t i = 0; i < header.entry_count; ++i) {
        ...
        if (result < 0) {
            ...
            std::fprintf(stderr, "  FAIL: %s — size mismatch (expected %llu, got %d)\n",
                         entry.name,
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if ((arg == "-l" || arg == "--list") && i + 1 < argc) {
            ...
            if (eq != std::string::npos) {
                ...
                entry.filepath = arg;
                entry.name     = normalize_name(fs::relative(entry.filepath, base_dir).string());
            }
    ...
    for (const auto& ce : compressed_entries)
        out.write(ce.compressed.data(), static_cast<std::streamsize>(ce.compressed.size()));
